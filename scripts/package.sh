...
for plugin in "${PLUGINS[@]}"; do
  ...
  m_ver=$(grep '^version\b' "$plugin/zeroclaw-plugin.toml" | head -1 | sed 's/.*= *"\(.*\)"/\1/')
  m_wasm=$(grep '^wasm_path\b' "$plugin/zeroclaw-plugin.toml" | head -1 | sed 's/.*= *"\(.*\)"/\1/')
  ...
  cp "$plugin/zeroclaw-plugin.toml"       "$staging/"
